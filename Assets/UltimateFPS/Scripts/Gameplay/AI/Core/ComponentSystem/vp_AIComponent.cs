...
public class vp_AIComponent : MonoBehaviour
{

	public bool Persist = false;
	protected vp_AIStateManager m_StateManager = null;
	public vp_EventHandler EventHandler = null;
	protected vp_AIState m_DefaultState = null;
	...
	public List<vp_AIState> States = new List<vp_AIState>();				// list of state presets for this vp_AIComponent
	public List<vp_AIComponent> Children = new List<vp_AIComponent>();		// list of child vp_AIComponents
	public List<vp_AIComponent> Siblings = new List<vp_AIComponent>();		// list of vp_AIComponents sharing the same gameobject
	public List<vp_AIComponent> Family = new List<vp_AIComponent>();		// list of vp_AIComponents sharing the same hierarchy
	public List<Renderer> Renderers = new List<Renderer>();				// list of Renderers in this and all child vp_AIComponents
	public List<AudioSource> AudioSources = new List<AudioSource>();	// list of AudioSources in this and all child vp_AIComponents
	...
	protected vp_AIState m_InitialState = null;		// used at editor runtime only
	public vp_AIState InitialState { get { return m_InitialState; } }
#endif

	public vp_AIStateManager StateManager { get { return m_StateManager; } }
	public vp_AIState DefaultState { get { return m_DefaultState; } }
	...
	protected virtual void Awake()
	{
		...
		m_StateManager = new vp_AIStateManager(this, States);
	...
	public void SetState(string state, bool enabled = true, bool recursive = false, bool includeDisabled = false)
	{
		...
		// scan the underlying hierarchy for vp_AIComponents and
		// set 'state' on every object found
		if (recursive)
		{
			foreach (vp_AIComponent c in Children)
			{
	...
	/// vp_AIComponent and all vp_AIComponents on the same gameobject.
	...
	public void ActivateGameObject(bool setActive = true)
	{
		...
		if (setActive)
		{
			...
			foreach (vp_AIComponent s in Siblings)
			{
				s.Activate();
			}
		}
	...
	public void RefreshDefaultState()
	{
		...
		vp_AIState defaultState = null;

		if (States.Count == 0)
		{
			// there are no states, so create default state
			defaultState = new vp_AIState(GetType().Name, "Default", null);
		...
		if (defaultState.Preset == null || defaultState.Preset.ComponentType == null)
			defaultState.Preset = new vp_AIComponentPreset();
	...
	public void RefreshInitialState()
	{
		...
		m_InitialState = null;
		m_InitialState = new vp_AIState(GetType().Name, "Internal_Initial", null);
		m_InitialState.Preset = new vp_AIComponentPreset();
	...
	/// settings
	/// </summary>
	public void ApplyPreset(vp_AIComponentPreset preset)
	{
		vp_AIComponentPreset.Apply(this, preset);
		RefreshDefaultState();
		Refresh();
	}
	...
	/// </summary>
	public vp_AIComponentPreset Load(string path)
	{
		vp_AIComponentPreset preset = vp_AIComponentPreset.LoadFromResources(this, path);
		RefreshDefaultState();
		Refresh();
		return preset;
	}
	...
	/// and refresh settings
	/// </summary>
	public vp_AIComponentPreset Load(TextAsset asset)
	{
		vp_AIComponentPreset preset = vp_AIComponentPreset.LoadFromTextAsset(this, asset);
		RefreshDefaultState();
		Refresh();
		return preset;
	}
	...
	/// <summary>
	/// inits the 'Children' list will all child vp_AIComponents
	/// </summary>
	public void CacheChildren()
	{

		Children.Clear();
		foreach (vp_AIComponent c in GetComponentsInChildren<vp_AIComponent>(true))
		{
	...
	/// <summary>
	/// inits the 'Siblings' list will all sibling vp_AIComponents
	/// </summary>
	public void CacheSiblings()
	{

		Siblings.Clear();
		foreach (vp_AIComponent c in GetComponents<vp_AIComponent>())
		{
			if (c != this)
				Siblings.Add(c);
		}
	...
	/// <summary>
	/// inits the 'Family' list will all vp_AIComponents existing
	...
	public void CacheFamily()
	{
		...
		Family.Clear();
		foreach (vp_AIComponent c in transform.root.GetComponentsInChildren<vp_AIComponent>(true))
		{
			if (c != this)
				Family.Add(c);
		}
	...
	/// in this plus all child vp_AIComponents
	/// </summary>
	...
	/// existing in this plus all child vp_AIComponents
	/// </summary>
	...
	/// <summary>
	/// activates the gameobject containing this vp_AIComponent.
	/// a virtual activation method provided for classes
	/// inheriting vp_AIComponent to add additional activation
	/// routines
	/// </summary>
	...
	/// <summary>
	/// deactivates the gameobject containing this vp_AIComponent.
	/// a virtual deactivation method provided for classes
	/// inheriting vp_AIComponent to add additional deactivation
	/// routines
	/// </summary>
